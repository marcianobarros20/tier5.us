...
<head>
	...
	<?php if( ! empty($smof_data['custom_favicon'])) { ?><link rel="shortcut icon" href="<?php echo esc_url($smof_data['custom_favicon']); ?>"><?php } ?>
...
$theme_version = str_replace('.', '-', $theme_version = us_get_main_theme_version());
$theme_class = 'us-theme_zephyr_'.$theme_version;
?>
<body <?php body_class('l-body '.$theme_class.$woocommerce_class); ?><?php echo  ($body_background_styles_text != '')?' style="'.$body_background_styles_text.'"':''; ?> ontouchstart="">
<?php
...
?>
<!-- CANVAS -->
<div class="l-canvas <?php echo esc_attr($layout_class.' '.$sidebar_position_class.$extended_header_class.$header_position_class.$lang_class); ?>">

	<!-- HEADER -->
	<div class="l-header<?php echo esc_attr($logo_pos_class.$header_transparent_class); ?>">

		<div class="l-subheader at_top"<?php if ( ! empty($smof_data['header_extra_height']) AND $smof_data['header_extra_height'] >= 36 AND $smof_data['header_extra_height'] <= 60) { echo ' style="line-height: '.esc_attr($smof_data['header_extra_height']).'px; "'; } ?>>
	...
	<span class="w-contacts-item-value"><a href="tel:<?php echo str_replace(array('-', '\\', ' ', '*'),'',$smof_data['header_phone']); ?>"><?php echo $smof_data['header_phone']; ?></a></span>
						</div>
					<?php } ?>
					...
					<?php if (@$smof_data['header_show_custom'] AND @$smof_data['header_custom_icon'] != '' AND @$smof_data['header_custom_text'] != '') { ?>
						<div class="w-contacts-item for_custom">
							<i class="fa fa-<?php echo esc_attr($smof_data['header_custom_icon']); ?>"></i>
					...
					if (@$smof_data['header_social_'.$social_key] != '')
					{
						...
						elseif ($social_key == 'google')
						{
							$output .= '<div class="w-socials-item gplus">
										<a class="w-socials-item-link" target="_blank" href="'.esc_url($smof_data['header_social_'.$social_key]).'">
		...
		<div class="l-subheader at_middle" <?php if ( ! empty($smof_data['header_main_height']) AND $smof_data['header_main_height'] >= 50 AND $smof_data['header_main_height'] <= 150) {
			...
			<div class="l-subheader-h i-widgets i-cf">
					...
					<a class="w-logo-link" href="<?php if (function_exists('icl_get_home_url')) echo icl_get_home_url(); else echo esc_url(home_url('/')); ?>">
						...
						<span class="w-logo-img" style="height: <?php echo (empty($smof_data['logo_height']))?30:$smof_data['logo_height'];?>px;">
							<?php if ( ! empty($smof_data['custom_logo'])): ?>
								<img class="for_default" src="<?php echo esc_url($smof_data['custom_logo']); ?>" alt="<?php bloginfo('name'); ?>" style="margin-bottom: -<?php echo (empty($smof_data['logo_height']))?30:$smof_data['logo_height'];?>px;">
							<?php endif; ?>
						...
						<div class="w-contacts">
							<div class="w-contacts-list">
								...
								<?php if (@$smof_data['header_email'] != '') { ?>
									<div class="w-contacts-item for_email">
										<i class="mdfi_communication_email"></i>
										<span class="w-contacts-item-value"><a href="mailto:<?php echo esc_url($smof_data['header_email']); ?>"><?php echo $smof_data['header_email']; ?></a></span>
									</div>
								<?php } ?>
								<?php if (@$smof_data['header_show_custom'] AND @$smof_data['header_custom_icon'] != '' AND @$smof_data['header_custom_text'] != '') { ?>
									<div class="w-contacts-item for_custom">
				...
				<div class="w-search">
					...
					<form class="w-search-form" action="<?php echo esc_url(home_url( '/' )); ?>">
				...
				<!-- NAV -->
				<nav class="w-nav layout_hor animation_<?php echo esc_attr($menu_hover_animation); ?>">
		...
		<div class="l-subheader at_bottom"<?php if ( ! empty($smof_data['header_extra_height']) AND $smof_data['header_extra_height'] >= 36 AND $smof_data['header_extra_height'] <= 60) { echo ' style="line-height: '.esc_attr($smof_data['header_extra_height']).'px; "'; } ?>>
			<div class="l-subheader-h i-cf">
				...
				<?php if (@$smof_data['header_show_search'] AND (@$smof_data['main_header_layout'] == 'advanced')) { ?>
					<div class="w-search">
						<span class="w-search-show"><i class="mdfi_action_search"></i></span>
						<div class="w-search-form-overlay"></div>
						<form class="w-search-form show_hidden" action="<?php echo esc_url(home_url( '/' )); ?>">
							<div class="w-search-form-h">
								<div class="w-search-form-row">
									<?php if (defined('ICL_LANGUAGE_CODE') AND ICL_LANGUAGE_CODE != '') { ?><input type="hidden" name="lang" value="<?php echo esc_attr(ICL_LANGUAGE_CODE); ?>"><?php } ?>
									<div class="w-search-label">
